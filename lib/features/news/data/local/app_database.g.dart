    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `articles` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `author` TEXT, `title` TEXT, `description` TEXT, `url` TEXT, `urlToImage` TEXT, `publishedAt` TEXT, `content` TEXT)');
...
class _$ArticleDao extends ArticleDao {
  ...
  Future<void> deleteArticle(String url) async {
    await _queryAdapter
        .queryNoReturn('DELETE FROM articles WHERE url = ?1', arguments: [url]);
  }
